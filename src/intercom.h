...
typedef struct __attribute__((__packed__)) {
	...
	// after this a dynamic buffer is appended to hold TLV - currently just
	// an ipv6 address is allowed
...
typedef struct __attribute__((__packed__)) {
	intercom_packet_hdr hdr;
	// after this a dynamic buffer is appended for plat info and basic
	// client info
...
typedef struct { uint8_t mac[ETH_ALEN]; } mac;
...
typedef struct __attribute__((__packed__)) {
	...
	// afterwards an array of elements of type intercom_packet_info_entry is
	// expected
