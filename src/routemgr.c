...
static void rtnl_change_address(routemgr_ctx *ctx, struct in6_addr *address,
				int type, int flags);
static void rtnl_handle_link(const struct nlmsghdr *nh);
static int rtnl_addattr(struct nlmsghdr *n, int maxlen, int type, void *data,
			int datalen);
...
void rtmgr_client_remove_address(struct in6_addr *dst_address) {
	...
	if (clientmgr_is_known_address(&l3ctx.clientmgr_ctx, dst_address,
				       &_client)) {
		clientmgr_remove_address(&l3ctx.clientmgr_ctx, _client,
					 dst_address);
		for (int i = 0; i < VECTOR_LEN(_client->addresses); i++) {
			routemgr_probe_neighbor(
			    &l3ctx.routemgr_ctx, _client->ifindex,
			    &VECTOR_INDEX(_client->addresses, i).addr,
			    _client->mac);
		}
	}
}
...
void rtnl_handle_neighbour(routemgr_ctx *ctx, const struct nlmsghdr *nh) {
	...
	parse_rtattr(tb, NDA_MAX, NDA_RTA(msg),
		     nh->nlmsg_len - NLMSG_LENGTH(sizeof(*msg)));

	if (!(ctx->clientif_index == msg->ndm_ifindex ||
	      ctx->client_bridge_index == msg->ndm_ifindex))
		return;
	...
	if (nh->nlmsg_type == RTM_NEWNEIGH && msg->ndm_state & NUD_REACHABLE &&
		...
		log_debug(
		    ...
		    "client-mac [%s]\n",
		    mac_str);
		clientmgr_notify_mac(CTX(clientmgr), RTA_DATA(tb[NDA_LLADDR]),
				     msg->ndm_ifindex);
	}
	...
	log_debug(
	    ...
	    mac_str, ip_str, if_indextoname(msg->ndm_ifindex, ifname),
	    nh->nlmsg_type, msg->ndm_state, msg->ndm_ifindex,
	    ctx->clientif_index,
	    ctx->client_bridge_index);  // see include/uapi/linux/neighbour.h
					// NUD_REACHABLE for numeric values

	if (msg->ndm_state & NUD_REACHABLE) {
		if (nh->nlmsg_type == RTM_NEWNEIGH && tb[NDA_DST] &&
		    tb[NDA_LLADDR]) {
			log_debug(
			    "Status-Change to NUD_REACHABLE, ADDING address %s "
			    "[%s]\n",
			    ip_str, mac_str);
			clientmgr_add_address(CTX(clientmgr), &dst_address,
		...
		if (nh->nlmsg_type == RTM_NEWNEIGH) {  // TODO: re-try sending
						       // NS if no NA is
						       // received
			if (clientmgr_valid_address(&l3ctx.clientmgr_ctx,
						    &dst_address)) {
				log_debug(
...
void client_bridge_changed(const struct nlmsghdr *nh,
	...
	if (!strncmp(ifname, l3ctx.routemgr_ctx.client_bridge,
		     strlen(ifname))) {
		parse_rtattr(tb, IFLA_MAX, IFLA_RTA(msg),
			     nh->nlmsg_len - NLMSG_LENGTH(sizeof(*msg)));
		...
		if (!memcmp(RTA_DATA(tb[IFLA_ADDRESS]),
			    l3ctx.routemgr_ctx.bridge_mac, 6)) {
			log_debug(
		...
		switch (nh->nlmsg_type) {
			case RTM_NEWLINK:
				log_verbose(
				    ...
				    print_mac(RTA_DATA(tb[IFLA_ADDRESS])),
				    ifname);
				clientmgr_notify_mac(&l3ctx.clientmgr_ctx,
...
void rtnl_handle_msg(routemgr_ctx *ctx, const struct nlmsghdr *nh) {
	...
	switch (nh->nlmsg_type) {
		case RTM_NEWROUTE:
			//		case RTM_DELROUTE:
			log_debug(
			    "handling netlink message for route change\n");
			handle_kernel_routes(ctx, nh);
			break;
		case RTM_NEWNEIGH:
		case RTM_DELNEIGH:
			log_debug(
			    "handling netlink message for neighbour change\n");
...
void routemgr_init(routemgr_ctx *ctx) {
	...
	for (int i = 0; i < VECTOR_LEN(CTX(clientmgr)->prefixes); i++) {
		struct prefix *prefix =
		    &(VECTOR_INDEX(CTX(clientmgr)->prefixes, i));
		log_verbose(
		...
		if (prefix->isv4) {
			...
			log_verbose("ipv4: %s\n", print_ip4(&ip4));
			routemgr_insert_route4(
			    ctx, 254, if_nametoindex(CTX(ipmgr)->ifname), &ip4,
			    prefix->plen - 96);
		} else
			routemgr_insert_route(
			    ctx, 254, if_nametoindex(CTX(ipmgr)->ifname),
			    (struct in6_addr *)(prefix->prefix.s6_addr),
			    prefix->plen);
	}
...
int parse_kernel_route_rta(struct rtmsg *rtm, int len,
	...
	for (struct rtattr *rta = RTM_RTA(rtm); RTA_OK(rta, len);
	     rta = RTA_NEXT(rta, len)) {
		switch (rta->rta_type) {
			case RTA_DST:
				...
				if (rtm->rtm_family == AF_INET6) {
					...
					memcpy(route->prefix.s6_addr,
					       RTA_DATA(rta), 16);
					log_debug(
					    "parsed route, found dst: %s\n",
			...
			case RTA_SRC:
				if (rtm->rtm_family == AF_INET6) {
					...
					memcpy(route->src_prefix.s6_addr,
					       RTA_DATA(rta), 16);
				} else if (rtm->rtm_family == AF_INET) {
					struct in_addr ipv4;
					memcpy(&ipv4.s_addr, RTA_DATA(rta), 4);
			...
			case RTA_GATEWAY:
				if (rtm->rtm_family == AF_INET6) {
					memcpy(route->gw.s6_addr, RTA_DATA(rta),
					       16);
				} else if (rtm->rtm_family == AF_INET) {
					struct in_addr ipv4;
					memcpy(&ipv4.s_addr, RTA_DATA(rta), 4);
	...
	while (1) {
		...
		if (NLMSG_OK(nh, count)) {
			switch (nh->nlmsg_type) {
				...
				case NLMSG_ERROR:
					perror(
					    "handling netlink error-message");
					ne = NLMSG_DATA(nh);
					if (ne->error <= 0)
						continue;
				/* Falls through. */
...
int rtnl_addattr(struct nlmsghdr *n, int maxlen, int type, void *data,
		 int datalen) {
...
void rtnl_add_address(routemgr_ctx *ctx, struct in6_addr *address) {
	...
	rtnl_change_address(ctx, address, RTM_NEWADDR,
			    NLM_F_CREATE | NLM_F_EXCL | NLM_F_REQUEST);
}

void rtnl_remove_address(routemgr_ctx *ctx, struct in6_addr *address) {
	rtnl_change_address(ctx, address, RTM_DELADDR,
			    NLM_F_REQUEST | NLM_F_ACK);
}

void rtnl_change_address(routemgr_ctx *ctx, struct in6_addr *address, int type,
			 int flags) {
	struct {
		struct nlmsghdr nl;
	...
	rtnl_addattr(&req.nl, sizeof(req), IFA_LOCAL, address,
...
void routemgr_probe_neighbor(routemgr_ctx *ctx, const int ifindex,
	...
	rtnl_addattr(&req.nl, sizeof(req), NDA_LLADDR, mac,
		     sizeof(uint8_t) * 6);
...
void routemgr_insert_neighbor(routemgr_ctx *ctx, const int ifindex,
	...
	rtnl_addattr(&req.nl, sizeof(req), NDA_DST, (void *)address,
		     sizeof(struct in6_addr));
	rtnl_addattr(&req.nl, sizeof(req), NDA_LLADDR, mac,
		     sizeof(uint8_t) * 6);
...
void routemgr_insert_route(routemgr_ctx *ctx, const int table,
	...
	rtnl_addattr(&req.nl, sizeof(req), RTA_DST, (void *)address,
		     sizeof(struct in6_addr));
	rtnl_addattr(&req.nl, sizeof(req), RTA_OIF, (void *)&ifindex,
		     sizeof(ifindex));
...
void routemgr_remove_route(routemgr_ctx *ctx, const int table,
	...
	rtnl_addattr(&req1.nl, sizeof(req1), RTA_DST, (void *)address,
		     sizeof(struct in6_addr));
	...
	rtnl_addattr(&req2.nl, sizeof(req2), RTA_DST, (void *)address,
		     sizeof(struct in6_addr));
...
void routemgr_remove_route4(routemgr_ctx *ctx, const int table,
			    struct in_addr *address, const int plen) {
	struct nlrtreq req1 = {
	    .nl =
		{
		    .nlmsg_type = RTM_NEWROUTE,
	...
	rtnl_addattr(&req1.nl, sizeof(req1), RTA_DST, (void *)&address[12],
		     sizeof(struct in_addr));
	rtmgr_rtnl_talk(ctx, (struct nlmsghdr *)&req1);
